classdef MultiDistanceProvider < DistanceProvider
    ...
    methods
        function obj = MultiDistanceProvider(P)
...
%                             ));
        ...
        function d = distance(obj,S1,S2,model)
            ...
            if obj.Settings.UseL2
                for k=1:D
                    d(:,:,counter) = ...
                        d(:,:,counter) + dist_L2_sqr(:,:,k).^(1 / obj.Settings.P);
                end
                counter = counter + 1;
            end
            ...
            if obj.Settings.UseW12
                for i=1:D
                    d(:,:,counter) = ...
                        d(:,:,counter) ...
                            + dist_L2_sqr(:,:,i).^(1 / obj.Settings.P);
                    for j=1:K
                        d(:,:,counter) = ...
                            d(:,:,counter) ...
                                + dist_L2_sqr_deriv(:,:,i,j).^(1 / obj.Settings.P);
                    end
                end
                counter = counter + 1;
            end
            ...
            if obj.Settings.UseW12Prime
                ...
                d(:,:,counter) = d(:,:,counter).^(1 / obj.Settings.P);
                counter = counter + 1;
            end
            if obj.Settings.UseW1infPrime
                for i=1:D
