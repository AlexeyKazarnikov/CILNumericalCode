...
#include "utils/FDSUtils.h"
#include "utils/NeumannBCIndexing.h"
	...
	value_type& t_dphidt)
{
	...
	auto kp = (t_k < t_par.N - 1) * (t_k + 1) + (t_k == t_par.N - 1) * t_k;
	auto km = (t_k >= 1) * (t_k - 1);

	auto phi_k = t_global_phi[t_k];
	auto phi_k_m = t_global_phi[km];
	auto phi_k_p = t_global_phi[kp];

	auto u_k = t_global_u[t_k];
	auto u_k_m = t_global_u[km];
	auto u_k_p = t_global_u[kp];

	value_type u_k_pp, u_k_mm;

	if (t_k <= t_par.N - 3)
		u_k_pp = t_global_u[t_k + 2];
	else
		u_k_pp = 2 * u_k_p - 2 * t_global_u[t_k - 1] + t_global_u[t_k - 2];

	if (t_k >= 2)
		u_k_mm = t_global_u[t_k - 2];
	else
		u_k_mm = t_global_u[t_k + 2] - 2 * t_global_u[t_k + 1] + 2 * u_k_m;

	du = (u_k - u_k_m) * t_par.dxinv;
	d2u = (u_k_p - 2 * u_k + u_k_m) * t_par.dx2inv;
	d3u = (u_k_pp - 2 * u_k_p + 2 * u_k_m - u_k_mm) * (value_type(0.5) * t_par.dx3inv);
	d4u = (u_k_pp - 4 * u_k_p + 6 * u_k - 4 * u_k_m + u_k_mm) * t_par.dx4inv;

	dphi = (phi_k - phi_k_m) * t_par.dxinv;
	d2phi = (phi_k_p - 2 * phi_k + phi_k_m) * t_par.dx2inv;
	...
	value_type* jac)
{
	...
	{
		if ((eid + threadIdx.y < 2) || (eid + threadIdx.y >= par.N + 2))
			return;
	...
	{
		if ((eid + threadIdx.y < 6) || (eid + threadIdx.y >= par.N + 6))
			return;
