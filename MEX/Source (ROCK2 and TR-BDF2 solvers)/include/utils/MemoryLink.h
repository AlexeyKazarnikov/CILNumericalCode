...
public:
	...
	T* data();
	const T* data() const;
	T& operator [] (const size_t t_index);
	const T& operator [] (const size_t t_index) const;
	T* begin();
	T* end();
	const T* begin() const;
	const T* end() const;
...
template<typename T> inline T* MemoryLink<T>::data()
{
	return this->m_data;
}

template<typename T> inline const T* MemoryLink<T>::data() const
{
	return this->m_data;
}

template<typename T> inline T & MemoryLink<T>::operator[](const size_t t_index)
{
...
template<typename T> inline const T& MemoryLink<T>::operator[](const size_t t_index) const
{
	if (t_index >= this->m_size)
		throw std::out_of_range("The index was out of range!");
	return this->m_data + t_index;
