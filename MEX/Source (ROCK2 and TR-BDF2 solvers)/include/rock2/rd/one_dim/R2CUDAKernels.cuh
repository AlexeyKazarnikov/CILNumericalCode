# pragma once
/*
* This header contains CUDA kernels, specific for one-dimensional two-component reaction-diffusion systems
*/
...
{
    auto jprev = (gid == 0) * gid + (gid > 0) * (gid - 1);
    auto jnext = (gid == grid_resolution - 1) * gid + (gid < grid_resolution - 1) * (gid + 1);

    u = y[gid];
    v = y[gid + grid_resolution];

    rhs_u = dc_nu1_hinv2 * (y[jprev] + y[jnext] - 2 * u) + f(u, v, dc_par);
    rhs_v = dc_nu2_hinv2 * (y[jprev + grid_resolution] + y[jnext + grid_resolution] - 2 * v) + g(u, v, dc_par);
}
...
)
{
    ...
    auto mid = fid / grid_resolution; // index of model which is processed
    auto gid = fid % grid_resolution; // index of the node in spatial grid, which is being processed
    auto sys_size = 2 * grid_resolution;
    ...
    if (mid < sim_number) // actual number of blocks does not necessarily correspond to the grid resolution
    {
        ...
        // yjm2 = y_prev
        yjm2[gid] = y_prev[gid];
        yjm2[gid + grid_resolution] = y_prev[gid + grid_resolution];
        ...
        value_type v = y_prev[gid + grid_resolution];

        value_type rhs_u = dc_nu1_hinv2 * (y_prev[jprev] + y_prev[jnext] - 2 * u) + f(u, v, dc_par);
        value_type rhs_v = dc_nu2_hinv2 * (y_prev[jprev + grid_resolution] + y_prev[jnext + grid_resolution] - 2 * v) + g(u, v, dc_par);

        value_type out_u = rhs_u * time_step * dc_recf[pos_recf] + u;
        value_type out_v = rhs_v * time_step * dc_recf[pos_recf] + v;
        yjm1[gid] = out_u;
        yjm1[gid + grid_resolution] = out_v;

        // if mdeg < 2 y = yjm1;
        if (degree < 2)
        {
            y[gid] = out_u;
            y[gid + grid_resolution] = out_v;
        }
    } // if
}
...
)
{
    ...
    eval_model_rhs(yjm1, rhs_u, rhs_v, u, v, gid, t_grid_resolution);

    y[gid] = temp1 * rhs_u + temp2 * u + temp3 * yjm2[gid];
    y[gid + t_grid_resolution] = temp1 * rhs_v + temp2 * v + temp3 * yjm2[gid + t_grid_resolution];
}
...
)
{
    ...
    eval_model_rhs(y, yjm2_u, yjm2_v, y_u, y_v, gid, grid_resolution);

    // yjm2 = yjm2[reg]
    yjm2[gid] = yjm2_u;
    yjm2[gid + grid_resolution] = yjm2_v;

    // yjm1 = y[reg] + temp1*yjm2[reg]; 
    yjm1[gid] = y_u + temp1 * yjm2_u;
    yjm1[gid + grid_resolution] = y_v + temp1 * yjm2_v;
}
...
)
{
    ...
    eval_model_rhs(yjm1, y_u, y_v, yjm1_u, yjm1_v, gid, grid_resolution);
    ...
    value_type temp3_v = temp2 * (y_v - yjm2[gid + grid_resolution]);

    y[gid] = yjm1_u + temp1 * y_u + temp3_u;
    y[gid + grid_resolution] = yjm1_v + temp1 * y_v + temp3_v;

    yjm2[gid] = temp3_u;
    yjm2[gid + grid_resolution] = temp3_v;
}
...
__global__ void spectral_radius_est_stage(
    ...
    value_type* t_rho,
    const index_type* t_run_indices,
    const index_type t_grid_resolution, // grid resolution
    const index_type t_sim_number // number of simulations
)
{
    ...
    auto gid = tid;
    while (gid < t_grid_resolution)
    {
        auto jprev = (gid == 0) * gid + (gid > 0) * (gid - 1);
        auto jnext = (gid == t_grid_resolution - 1) * gid + (gid < t_grid_resolution - 1) * (gid + 1);

        unsigned int lin_coeff = 2 - (jprev == gid) - (jnext == gid);

        auto center_u = -dc_nu1_hinv2 * lin_coeff + dfdu(y[gid], y[gid + t_grid_resolution], dc_par);
        auto center_v = -dc_nu2_hinv2 * lin_coeff + dgdv(y[gid], y[gid + t_grid_resolution], dc_par);
        auto radius_u = dc_nu1_hinv2 * lin_coeff + abs(dfdv(y[gid], y[gid + t_grid_resolution], dc_par));
        auto radius_v = dc_nu2_hinv2 * lin_coeff + abs(dgdu(y[gid], y[gid + t_grid_resolution], dc_par));
